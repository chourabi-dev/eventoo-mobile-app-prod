...
import 'package:mobile/models/networking_invitation_data_model.dart';
import 'package:mobile/models/participant_model.dart';
import 'package:mobile/services/event_service.dart';

class MeetingInvitationReplanifierDialog extends StatefulWidget {
  final int invitationID;
  const MeetingInvitationReplanifierDialog({super.key, required this.invitationID});

  @override
  State<MeetingInvitationReplanifierDialog> createState() =>
      _MeetingInvitationReplanifierDialogState();
}

class _MeetingInvitationReplanifierDialogState extends State<MeetingInvitationReplanifierDialog> {
  ...
  NetworkingInvitationDataModel? invitation;
  ...
  void fetchInvitationDATA(){
    eventService.fetchInvitationNativeDATA(widget.invitationID).then((res){
      dynamic body = jsonDecode(res.body);

      print(body);

      setState(() {
        invitation = NetworkingInvitationDataModel.fromJson(body['data']);

        print(invitation!.senderId);
  ...
  void fetchTimeSlots(){  
    ...
    eventService.networkingTimeSlots(invitation!.senderId, int.parse(_selectedDate!.id) ).then((res){
      dynamic body = jsonDecode(res.body);
  ...
  void fetchLoacations(){
    eventService.networkingLocations(invitation!.senderId ).then((res){
      dynamic body = jsonDecode(res.body);
...
void showErrorDialog(BuildContext context, String message) {
  ...
  void _submit() {
    if (_selectedDate == null ||
      ...
      eventService.networkingReSendInvitation(widget.invitationID, int.parse(_selectedDate!.id) ,  _selectedTime!, int.parse(_selectedLocation!.id)  ).then((res){
        dynamic body = jsonDecode(res.body);
